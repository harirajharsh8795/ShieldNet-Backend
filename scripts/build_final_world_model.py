...
def main():
    ...
    criterion = WorldModelLoss(
        ...
        lambda_mitre=0.25,
        lambda_order=0.5,
        focal_gamma=0.0,  # Standard weighted cross-entropy
    ...
    print(f"\nTraining World Model for {epochs} epochs on {device}...", flush=True)
    ...
    threshold_evals = []
    for thresh in [0.50, optimal_threshold, 0.25]:
        ...
        mode_name = "Default Balanced (0.50)" if thresh == 0.50 else ("Optimal PR-F1" if thresh == optimal_threshold else "SOC High-Recall Sentinel (0.25)")
    ...
    print(f"  {'Operating Mode':32s} | {'Threshold':9s} | {'Attack Recall':13s} | {'Precision':10s} | {'Binary F1':9s} | {'FPR':7s} | {'Attacks Caught':14s}", flush=True)
    print("  " + "-" * 104, flush=True)
    for te in threshold_evals:
        total_attacks = te['true_positives'] + te['false_negatives']
        print(f"  {te['mode']:32s} | {te['threshold']:9.4f} | {te['attack_recall']:13.2%} | {te['precision']:10.2%} | {te['binary_f1']:9.4f} | {te['false_positive_rate']:7.2%} | {te['true_positives']:3d} / {total_attacks:3d} ({te['attack_recall']:.1%})", flush=True)
        
    # ─── 4. PER-CLASS F1 BREAKDOWN TABLE ──────────────────────────────────────
    print("\n" + "=" * 80, flush=True)
    print("PER-CLASS CLASSIFICATION BREAKDOWN (ATTENTION-GRU WORLD MODEL)", flush=True)
    print("=" * 80, flush=True)
    ...
    final_summary = {
        ...
        "loss_function": "MSE (State) + CE (Class, alpha-balanced) + CE (MITRE) + BCE (Order)",
        "ordered_metrics": {
            "macro_f1": ord_f1,
